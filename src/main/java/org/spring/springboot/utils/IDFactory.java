...
public class IDFactory {
     ...
     */
      ...
      */
      ...
      private IDFactory() {
         ...
         LOGGER.info("IDFactory: " + getUsedCount() + " id's used.");
     }
    ...
    public int nextId() {
       try {
           ...
           if (id == Integer.MIN_VALUE) {
                   LOGGER.error("All id's are used!",new Exception("All id's are used!"));
               }
           idList.set(id);
      ...
      private void lockIds(int... ids) {
         try {
          ...
          for (int id : ids) {
                  ...
                  if (status) {
                      LOGGER.error("ID " + id + " is already taken, fatal error!",new Exception("ID " + id + " is already taken, fatal error!"));
                  }
              idList.set(id);
              }
      } finally {
   ...
   public void releaseId(int id) {
       try {
               ...
               boolean status = idList.get(id);
               if (!status) {
                LOGGER.error("ID " + id + " is not taken, can't release it.",new Exception("ID " + id + " is not taken, can't release it."));
            }
        idList.clear(id);
